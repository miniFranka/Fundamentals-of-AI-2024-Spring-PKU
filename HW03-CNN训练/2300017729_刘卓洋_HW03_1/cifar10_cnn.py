...
#例如，如果你还需要对图像进行其他的预处理操作，比如裁剪、归一化、翻转等，可以这样做：
'''
...
'''
#在这个例子中，transform 会依次执行Resize、ToTensor和Normalize三个操作，对图片进行预处理。
...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        #在实例化 nn.Conv2d(3, 6, 5) 后，self.conv1 将会在前向传播过程中对输入的三维张量（batch_size x 3 x height x width）应用 5x5 的卷积操作，并生成新的三维张量（batch_size x 6 x new_height x new_width）。
        #self.conv1 = nn.Conv2d(3, 64, 5,1,0,1)
        #输入的训练图像是32x32
        ...
        #第一个参数 16 * 5 * 5 表示输入张量的大小，这里表示输入张量大小为 16 * 5 * 5。
        #第二个参数 120 表示输出张量的大小，这里表示输出张量大小为 120。
        #第三个参数 bias 表示是否使用偏置（bias），这里表示不使用偏置。
        #在实例化 nn.Linear(16 * 5 * 5, 120) 后，self.fc1 将会在前向传播过程中对输入的一维张量（batch_size x 16 * 5 * 5）应用线性变换，并生成新的一维张量（batch_size x 120）。
