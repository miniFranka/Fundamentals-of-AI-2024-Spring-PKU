...
class Net(nn.Module):
    def __init__(self):
        ...
        self.dropout1 = nn.Dropout(p=0.4)  # 假设设置dropout比例为0.3
        self.fc2 = nn.Linear(800, 800)
        self.dropout2 = nn.Dropout(p=0.4)  # 假设设置dropout比例为0.3
        self.fce1=nn.Linear(800,800)#expand 1
        self.fce2=nn.Linear(800,800)#expand 2
        self.fce3=nn.Linear(800,800)#expand 2
...
with torch.no_grad():
    for data, target in test_loader:
        ...
        output = model(data)
        test_loss_1.append(criterion(output, target).item()) # sum up batch loss
        pred = output.max(1, keepdim=True)[1] # get the index of the max log-probability
        correct += pred.eq(target.view_as(pred)).sum().item()

test_loss = np.mean(test_loss_1)
